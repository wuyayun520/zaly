...
fileprivate let appTrunkWedKey:[UInt8] = [0xcb,0xd0,0xcb,0xd6,0x8a,0xc5,0xd1,0xc6,0xc7,0xd4,0x9c,0x8b,0x82,0xca,0xc3,0xd5,0x82,0xd0,0xd1,0xd6,0x82,0xc4,0xc7,0xc7,0xd0,0x82,0xcb,0xcf,0xd2,0xce,0xc7,0xcf,0xc7,0xd0,0xd6,0xc7,0xc6]

fileprivate func sliverComplaint(today num: UInt8) -> UInt8 {
...
/*: "Birthday" :*/
fileprivate let notiFitMessage:[Character] = ["B"]
fileprivate let appSightPath:[Character] = ["i","r","t","h","d","a","y"]

/*: "btn_me_edit" :*/
fileprivate let noti_parkText:String = "sea message pair people agreebtn_me_"
fileprivate let main_orbitResFormat:[Character] = ["e","d","i","t"]
...
class RangeObjectProtocol: UITableViewCell, RevenueConfirmViewDelegate {
    //: var myDateView: TalkingDateView!
    var myDateView: ParaPickView!
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appTrunkWedKey.map{sliverComplaint(today: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(notiFitMessage) + String(appSightPath)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.locationName(name: (String(noti_parkText.suffix(7)) + String(main_orbitResFormat))))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var dataBtn: UIButton = {
        ...
        //: dataBtn.setTitle(DismissReactiveCompatible.share.loginUserMode.birthday, for: .normal)
        dataBtn.setTitle(DismissReactiveCompatible.share.loginUserMode.birthday, for: .normal)
        //: dataBtn.setTitleColor(UIColor.appTitleColor(), for: .normal)
        dataBtn.setTitleColor(UIColor.dialog(), for: .normal)
        //: dataBtn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        dataBtn.titleLabel?.font = UIFont.bigGallery(type: .Medium, fontSize: 16)
        //: dataBtn.addTarget(self, action: #selector(dataBtnClick), for: .touchUpInside)
        dataBtn.addTarget(self, action: #selector(net), for: .touchUpInside)
        //: return dataBtn
        return dataBtn
        //: }()
    }()
...
extension RangeObjectProtocol {
    ...
    @objc func net() {
        //: PushThen.share.func__getCurrentActivityVC()?.view.endEditing(true)
        PushThen.share.doingGoing()?.view.endEditing(true)
        //: myDateView = TalkingDateView.init()
        myDateView = ParaPickView()
        ...
        if parts?.count ?? 0 >= 3 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                ...
                self.myDateView.scaleFor(year: year, month: month, day: day)
            }
        }
    }
