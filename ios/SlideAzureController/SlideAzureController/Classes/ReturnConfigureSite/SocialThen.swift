...
fileprivate let main_careAroundMsg:[UInt8] = [0xab,0xb0,0xab,0xb6,0x6a,0xa5,0xb1,0xa6,0xa7,0xb4,0x7c,0x6b,0x62,0xaa,0xa3,0xb5,0x62,0xb0,0xb1,0xb6,0x62,0xa4,0xa7,0xa7,0xb0,0x62,0xab,0xaf,0xb2,0xae,0xa7,0xaf,0xa7,0xb0,0xb6,0xa7,0xa6]

fileprivate func hiEdit(vid num: UInt8) -> UInt8 {
...
/*: "icon_rank_coin" :*/
fileprivate let data_pairName:[UInt8] = [0xec,0xe6,0xea,0xeb,0xda,0xf7,0xe4,0xeb,0xee,0xda,0xe6,0xea,0xec,0xeb]

private func wedLoad(her num: UInt8) -> UInt8 {
    return num ^ 133
}

/*: "icon_lounge" :*/
fileprivate let k_agoStr:String = "kingdomo"
fileprivate let const_helpData:[Character] = ["n","_","l","o","u","n","g","e"]

/*: "icon_rank_coin2" :*/
fileprivate let notiHarassmentText:[Character] = ["i","c","o","n","_","r","a"]
fileprivate let dataSuiteContent:String = "burn large opportunity starnk_"
fileprivate let app_hisMessage:String = "outsideoin2"
...
class SocialThen: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_careAroundMsg.map{hiEdit(vid: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.saveInColor()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .bigGallery(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = RangeReactiveCompatible().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.locationName(name: String(bytes: data_pairName.map{wedLoad(her: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .bigGallery(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.beforeCan(colors: UIColor.minimizeOn(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.locationName(name: (k_agoStr.replacingOccurrences(of: "kingdom", with: "ic") + String(const_helpData)))
    }
}
...
extension SocialThen {
    ...
    func nativeWarning(model: DisableModelType, index: Int) {
        ...
        headPic.bolt(urlStr: model.headPic)
    ...
    func flushOrVariation(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: data_pairName.map{wedLoad(her: $0)}, encoding: .utf8)! : (String(notiHarassmentText) + String(dataSuiteContent.suffix(3)) + app_hisMessage.replacingOccurrences(of: "outside", with: "c"))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.locationName(name: imageStr), for: .normal)
    }
}
...
extension SocialThen {
    ...
    private func lifeTimeConstraint() {
