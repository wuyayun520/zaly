...
fileprivate let show_responseValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
@objc protocol WeddingObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func courseAm()

    //: @objc optional func endScroll()
    @objc optional func ceaseScroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class PortraitViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(charmPhone))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_responseValue.reversed(), encoding: .utf8)!)
    }
...
extension PortraitViewDelegate {
    ...
    func currentTo() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(chin), userInfo: nil, repeats: true)
    }
...
extension PortraitViewDelegate {
    ...
    func cityInput(msgModel: FirAimModelType?) {
    ...
    func scroll() {
        ...
        if AllHeight > userSumReplaceFormat {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < userSumReplaceFormat {
                ...
                self.msgTableView.height = userSumReplaceFormat
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = userSumReplaceFormat - AllHeight
        ...
        electronicDatabase(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func electronicDatabase(animated: Bool) {
...
extension PortraitViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func pastSituation(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.ceaseScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kProgressId)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ArtRegularBassCell.transitionConnect(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PortraitViewDelegate {
    ...
    func appearSubviews() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: data_textValue, height: userSumReplaceFormat)
    }
}
