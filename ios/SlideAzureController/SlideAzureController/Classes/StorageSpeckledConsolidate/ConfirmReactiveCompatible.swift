...
fileprivate let noti_colorPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_loading_lose" :*/
fileprivate let notiWarnName:[Character] = ["i","m","g","_","l","o","a","d","i","n","g","_","l","o","s","e"]
...
class ConfirmReactiveCompatible: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_colorPath.reversed(), encoding: .utf8)!)
    }
...
extension ConfirmReactiveCompatible {
    ...
    private func filter() {
        ...
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(submit))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(install)).then {
...
extension ConfirmReactiveCompatible {
    ...
    func snub(imgModel: TillGrantMeasurable) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(notiWarnName))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.recoverShared()
            }
        }
    ...
    func current(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(notiWarnName))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.recoverShared()
        }
    }
...
extension ConfirmReactiveCompatible: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension ConfirmReactiveCompatible {
    ...
    func recoverShared() {
        ...
        var newW = main_logStr
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = main_logStr * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > main_nameData {
            //: newH = ScreenHeight
            newH = main_nameData
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * main_nameData / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (main_logStr - newW) / 2.0, y: (main_nameData - newH) / 2)
