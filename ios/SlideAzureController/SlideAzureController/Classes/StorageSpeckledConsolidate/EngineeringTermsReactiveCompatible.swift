...
fileprivate let notiNeverSaveText:[UInt8] = [0x47,0x40,0x47,0x5a,0x6,0x4d,0x41,0x4a,0x4b,0x5c,0x14,0x7,0xe,0x46,0x4f,0x5d,0xe,0x40,0x41,0x5a,0xe,0x4c,0x4b,0x4b,0x40,0xe,0x47,0x43,0x5e,0x42,0x4b,0x43,0x4b,0x40,0x5a,0x4b,0x4a]

private func destroyResource(demand num: UInt8) -> UInt8 {
    return num ^ 46
}
...
// __CLOSE_PRINT__
//
//  EngineeringTermsReactiveCompatible.swift
//  AbroadTalking
//
...
class EngineeringTermsReactiveCompatible: LovelyView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiNeverSaveText.map{destroyResource(demand: $0)}, encoding: .utf8)!)
    }
    ...
    override func launchDirection() {
        ...
        let filePath = WallAnimatTool.shared.streetwiseBecome(fileName: (self.giftEffectModel?.mainFile)!, model: self.effectMsgModel!)
        ...
        parser?.parse(with: data! as Data, cacheKey: key, completionBlock: { [weak self] videoItem in
            ...
            self.delegate?.colorYear(effectItemView: self, success: true)
...
extension EngineeringTermsReactiveCompatible: SVGAPlayerDelegate {
