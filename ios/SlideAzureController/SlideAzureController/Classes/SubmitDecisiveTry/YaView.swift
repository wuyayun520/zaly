...
/*: "#ffffff" :*/
fileprivate let mainConditionData:String = "weak ignore stack#ffffff"

/*: "earned" :*/
fileprivate let appWaitFormat:String = "surgery"
fileprivate let data_soundMsg:String = "aallowed"

/*: "Coins in" :*/
fileprivate let k_shouldName:String = "item"
fileprivate let user_tunStr:[Character] = ["o","i","n","s"," ","i","n"]

/*: "Error:  :*/
fileprivate let app_basicFormat:[Character] = ["E","r","r","o","r",":"," "]

/*: "#C946D2" :*/
fileprivate let constHandleData:[Character] = ["#","C","9","4","6","D","2"]

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let k_targetPath:String = "pan last gobtn_ch"
fileprivate let main_certainStr:String = "loatStwo running merely simultaneously convert"
fileprivate let noti_sweetName:String = "rBK_nobroad maybe"
fileprivate let kAgentNatureProductId:[Character] = ["r"]
...
class YaView: UIView {
    ...
    private let TextColor = (String(mainConditionData.suffix(7)))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: NaturalHandyJSON?) {
        //: self.init()
        self.init()
    ...
    private func discountModel() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            modifyActive(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (appWaitFormat.replacingOccurrences(of: "surgery", with: "e") + data_soundMsg.replacingOccurrences(of: "allow", with: "rn"))) {
                ...
                if let range = attrStr.string.range(of: (k_shouldName.replacingOccurrences(of: "item", with: "C") + String(user_tunStr))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.bigGallery(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(app_basicFormat)) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func modifyActive(withUrl url: String) {
        ...
        self.backgroundImageView.loadPoiseFinishParkImageEye(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.attributeGroupQuality(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: NaturalHandyJSON? {
        //: didSet {
        didSet {
            //: configModel()
            discountModel()
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(constHandleData)))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.locationName(name: (String(k_targetPath.suffix(6)) + "at_f" + String(main_certainStr.prefix(5)) + "_use" + String(noti_sweetName.prefix(6)) + String(kAgentNatureProductId)))
        //: return h
        return h
        //: }()
    }()
...
extension YaView {
    ...
    @objc func spark(_: UITapGestureRecognizer) {
        ...
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = DismissReactiveCompatible.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = DismissReactiveCompatible.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: PushThen.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        PushThen.share.plumeBright(urlStr: httpsUrl, webConfig: config)
    }
}
...
extension YaView {
    ...
    private func setupLogConfinement() {
