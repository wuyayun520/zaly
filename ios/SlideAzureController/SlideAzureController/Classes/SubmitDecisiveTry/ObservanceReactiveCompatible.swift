...
fileprivate let dataInvolvementTitle:[UInt8] = [0xec,0xeb,0xec,0xf1,0xad,0xe6,0xea,0xe1,0xe0,0xf7,0xbf,0xac,0xa5,0xed,0xe4,0xf6,0xa5,0xeb,0xea,0xf1,0xa5,0xe7,0xe0,0xe0,0xeb,0xa5,0xec,0xe8,0xf5,0xe9,0xe0,0xe8,0xe0,0xeb,0xf1,0xe0,0xe1]

private func questDoing(check num: UInt8) -> UInt8 {
    return num ^ 133
}

/*: "btn_dynamic_stop_nor" :*/
fileprivate let constFenceFormat:[Character] = ["b","t","n","_","d","y","n","a","m","i","c","_","s","t","o","p"]
fileprivate let kLikeName:String = "_norreserve transportation wave argument"

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let app_pageRankKey:String = "gold finishbtnwOb"
fileprivate let main_roadFormat:String = "tdj_omeet join"
fileprivate let main_betweenToneEndText:String = "_stopstall appearance assist"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let show_mouthPath:String = "btnCcapital currency"
fileprivate let constDonMessage:String = "_oedstar reminder come busy"
...
// __CLOSE_PRINT__
//
//  ObservanceReactiveCompatible.swift
//  AbroadTalking
//
...
protocol ViewTillThen: NSObject {
    //: func func__interactionViewStatus(view: TalkingVideoPlayerInteractionView, status: InteractionViewStatus)
    func tarWrap(view: ObservanceReactiveCompatible, status: SolarViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func licenseElementConnection(view: ObservanceReactiveCompatible, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class ObservanceReactiveCompatible: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: ViewTillThen?
    ...
    override init(frame: CGRect) {
        ...
        self.capTotaleraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataInvolvementTitle.map{questDoing(check: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.locationName(name: (String(constFenceFormat) + String(kLikeName.prefix(4)))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(want(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.locationName(name: (String(app_pageRankKey.suffix(6)) + "lEh_" + String(main_roadFormat.prefix(5)) + "ediv" + String(main_betweenToneEndText.prefix(5)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.locationName(name: (String(show_mouthPath.prefix(4)) + "mjoS7_tdj" + String(constDonMessage.prefix(4)) + "iv_play")), for: .selected)
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.locationName(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: RelativePlayerSlider = {
        ...
        let slid = RelativePlayerSlider(frame: CGRect(x: 100, y: 0, width: main_logStr - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension ObservanceReactiveCompatible {
    //: @objc func centerButtonClickAction(sender: UIButton) {
    @objc func want(sender: UIButton) {
        //: sender.isHidden = true
        sender.isHidden = true
        //: self.handleStatus(status: .StartPlay)
        self.feeStatus(status: .StartPlay)
    }
...
extension ObservanceReactiveCompatible {
    ...
    private func capTotaleraction() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.licenseElementConnection(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] _ in
            ...
            self.feeStatus(status: .StopSeek)
        }
    }
}
