...
fileprivate let userPointMaleLinkValue:[UInt8] = [0x61,0x66,0x61,0x6c,0x20,0x5b,0x67,0x5c,0x5d,0x6a,0x32,0x21,0x18,0x60,0x59,0x6b,0x18,0x66,0x67,0x6c,0x18,0x5a,0x5d,0x5d,0x66,0x18,0x61,0x65,0x68,0x64,0x5d,0x65,0x5d,0x66,0x6c,0x5d,0x5c]

fileprivate func periodOfTime(beautiful num: UInt8) -> UInt8 {
...
fileprivate let dataTunId:String = "TRTC 视load cold valid equal"
fileprivate let constDiskStr:String = "K发\u{9001}采集"
fileprivate let mainCosFormat:String = "的mini"
fileprivate let user_tapMessage:String = "频face片: "

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let main_tradeMessage:[Character] = ["L","I","V","E"," ","直","\u{64ad}","向","S","D","K","发","送"]
fileprivate let const_literalKey:String = "采\u{96c6}\u{7684}视频帧"
...
class EffectiveThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userPointMaleLinkValue.map{periodOfTime(beautiful: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var captureView: OvalView = {
...
extension EffectiveThen {
    /// 开启本地预览
    /// - Parameters:
    ///   - isSmallWindow: 是否自己在小窗口
    ///   - livePusher: 是否用tx推流
    //: func startPreView(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
    func fragmentRating(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
        ...
        if self.captureView.superview == nil {
            ...
            self.captureView.stCamera.lastStart()
        }
        //: self.captureView.setupVideoConversationPreviewSize(isSmallWindow)
        self.captureView.picture(isSmallWindow)
    }
    ...
    func viewTable() {
        ...
        self.captureView.stCamera.stillCan()
    ...
    func usDimension(isFront _: Bool) {
        //: guard SenseTime_Use == true else { return }
        guard main_dismissMessage == true else { return }
        //: guard self.captureView.stCamera != nil else {
        guard self.captureView.stCamera != nil else {
...
extension EffectiveThen: OverwhelmDelegete {
    ...
    func view(of _: OvalView!, post sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = platformImage(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                MateThen.trackBy(msg: (String(dataTunId.prefix(6)) + "频通\u{8bdd}向SD" + constDiskStr + mainCosFormat.replacingOccurrences(of: "mini", with: "视") + user_tapMessage.replacingOccurrences(of: "face", with: "帧图")) + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
    ...
    func flat(_: OvalView!, media sampleBuffer: CMSampleBuffer!) {
        //: if livePusher != nil {
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = platformImage(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        MateThen.trackBy(msg: (String(main_tradeMessage) + const_literalKey + "图片: ") + "\(image.size)")
                    }
                }
    ...
    func platformImage(_ imageBuffer: CVImageBuffer) -> UIImage? {
