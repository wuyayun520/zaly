...
fileprivate let app_herUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "gift_bg_dm" :*/
fileprivate let kSwingMsg:String = "GI"
fileprivate let mainResourceFormat:String = "ft_blifetime check fragment burst check"

/*: "#FFF4B5" :*/
fileprivate let const_impulseHelloMessage:String = "#Ffair require welcome up"
fileprivate let main_justMessage:[Character] = ["F","F","4","B","5"]

/*: "won " :*/
fileprivate let const_leaseData:[Character] = ["w","o","n"," "]

/*: " coins in " :*/
fileprivate let mainLatFormat:[Character] = [" ","c","o","i","n"]
fileprivate let mainErrUrl:[Character] = ["s"," ","i","n"," "]

/*: "just sent" :*/
fileprivate let data_neckUrl:String = "naturalust"

/*: "#4120C1" :*/
fileprivate let userFlagSlideTitle:String = "#4120Cinform button evaluate"
fileprivate let k_unitValue:String = "1"
...
// __CLOSE_PRINT__
//
//  WithholdReactiveCompatible.swift
...
class WithholdReactiveCompatible: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = SocialTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_herUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.locationName(name: (kSwingMsg.lowercased() + String(mainResourceFormat.prefix(4)) + "g_dm"))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(const_impulseHelloMessage.prefix(2)) + String(main_justMessage)))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(const_impulseHelloMessage.prefix(2)) + String(main_justMessage)))
...
extension WithholdReactiveCompatible {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func explainGift(_ model: SocialTransformable) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.bolt(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(const_leaseData)).localized + "\(giftModel.totalCoin)" + (String(mainLatFormat) + String(mainErrUrl)).localized + giftModel.gameName
            ...
            giftImgV.bolt(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (data_neckUrl.replacingOccurrences(of: "natural", with: "j") + " sent").localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = beard(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(userFlagSlideTitle.prefix(6)) + k_unitValue.capitalized))!, range: nsRange)
            }
    ...
    private func beard(giftmodel: SocialTransformable) -> String {
...
extension WithholdReactiveCompatible {
    ...
    private func unopen() {
        ...
        self.frame = CGRect(x: 0, y: constAgentJumpValue, width: main_logStr, height: 60)
