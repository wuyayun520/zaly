...
fileprivate let notiObserverData:[UInt8] = [0x7e,0x79,0x7e,0x63,0x3f,0x74,0x78,0x73,0x72,0x65,0x2d,0x3e,0x37,0x7f,0x76,0x64,0x37,0x79,0x78,0x63,0x37,0x75,0x72,0x72,0x79,0x37,0x7e,0x7a,0x67,0x7b,0x72,0x7a,0x72,0x79,0x63,0x72,0x73]

private func materialUnknown(income num: UInt8) -> UInt8 {
    return num ^ 23
}

/*: "More" :*/
fileprivate let appEstimatedUrl:String = "Morecenter drag link"

/*: "icon_details_arrow" :*/
fileprivate let kAppealStr:[Character] = ["i","c","o","n","_","d","e","t","a","i","l","s","_","a","r","r","o"]
fileprivate let k_selectionKey:[Character] = ["w"]

/*: "Post" :*/
fileprivate let appMaybePath:String = "Postnatural network"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let k_soccerKey:[Character] = ["b","t","n","_","d","y","n","a","m"]
fileprivate let dataLivingName:String = "warn vid energy entity soundic_sto"
...
class LimeThen: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.scriptConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiObserverData.map{materialUnknown(income: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .searched(0, 0, 0, 0.4)
    ...
    lazy var moreButton: RangeReactiveCompatible = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .bigGallery(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(appEstimatedUrl.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(kAppealStr) + String(k_selectionKey))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(cling), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension LimeThen {
    ...
    func announcementPosition(userModel: DisableMeasurable) {
        ...
        strA = [(String(appMaybePath.prefix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.dialog(), UIColor.ownLight()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.pingfang(fontSize: 18), UIFont.pingfang(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: ParaNameureModelType = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.regularPolygon())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    scale(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.regularPolygon())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    scale(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.regularPolygon())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    scale(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func cling() {
        ...
        let vc = ValueRecognizerDelegate(uid: userId)
    ...
    func scale(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(k_soccerKey) + String(dataLivingName.suffix(6)) + "p_nor")))
...
extension LimeThen {
    ...
    private func scriptConstraint() {
        ...
        let height = (main_logStr - actualWidth(w: 15 * 4)) / 3.0
