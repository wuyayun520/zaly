...
/*: "Finish" :*/
fileprivate let data_scenarioShouldKey:String = "Finishtiming aid close"

/*: "Cancel" :*/
fileprivate let data_impulseFormat:String = "same"
fileprivate let kTagValue:String = "aconductcel"

/*: "btn_photo_choice_pre" :*/
fileprivate let user_givingUrl:String = "bmissing"
fileprivate let user_alreadyPath:String = "correct task_photo"
fileprivate let const_rawData:String = "ice_prematter description program military framework"

/*: "btn_photo_add_nor" :*/
fileprivate let kTradeLeadingFormat:String = "btn_phmove qualify invitation err birthday"
fileprivate let const_rocketOnerName:[Character] = ["o","t"]
fileprivate let main_countKey:String = "o_add_norleading report"

/*: "/tmp" :*/
fileprivate let data_spaceId:String = "post track flow delay/tmp"

/*: "Export session failed" :*/
fileprivate let appPlantFormat:String = "Excity there fact motion"
fileprivate let userEnjoyUrl:String = "writing movement work yetport "
fileprivate let main_shineTitle:String = "shared fingeron fa"

/*: "Export canceled" :*/
fileprivate let data_micId:String = "ply segment para para scenarioExpo"
fileprivate let main_insideValue:String = "climitedllimitedd"

/*: "Successful!" :*/
fileprivate let user_secureId:String = "burst scoreSuccess"
fileprivate let userBlockPath:[Character] = ["f","u","l","!"]

/*: "Unsupported video formats" :*/
fileprivate let user_expressMsg:[Character] = ["U","n","s","u","p","p","o","r","t","e","d"," ","v","i","d","e","o"," ","f","o","r","m"]
fileprivate let main_taMsg:[Character] = ["a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let constDestroyDoinglyData:String = "no okyyyy-"
fileprivate let main_unitTempMsg:String = "migration border strength gesture meetd-HH:m"
fileprivate let userFlushlyMsg:String = "m:ss-SSSlease list communication food"

/*: "/tmp/video- :*/
fileprivate let mainSpeedPrimaryContent:String = "/tmp/vipeer accommodate remote"
fileprivate let k_districtValue:String = "interested label ski qualifydeo-"

/*: .mp4" :*/
fileprivate let dataBasicStandUrl:String = ".mp4framework click explain data job"
...
class MedalPickTool: NSObject {
    ...
    class func sameMember(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = main_logStr * 488 / 375
            ...
            imagePicker?.cropRect = CGRect(x: 0, y: (main_nameData - height) / 2, width: main_logStr, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(data_scenarioShouldKey.prefix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (data_impulseFormat.replacingOccurrences(of: "same", with: "C") + kTagValue.replacingOccurrences(of: "conduct", with: "n")).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.bigGallery(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.ownLight()
        ...
        imagePicker?.naviTitleFont = UIFont.bigGallery(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.dialog()
        ...
        imagePicker?.iconThemeColor = UIColor.attractiveIn()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.locationName(name: (user_givingUrl.replacingOccurrences(of: "missing", with: "tn") + String(user_alreadyPath.suffix(6)) + "_cho" + String(const_rawData.prefix(7))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.locationName(name: (String(kTradeLeadingFormat.prefix(6)) + String(const_rocketOnerName) + String(main_countKey.prefix(9))))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.bigGallery(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(data_scenarioShouldKey.prefix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(data_scenarioShouldKey.prefix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.attractiveIn()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: main_logStr - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func thanCompletion(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(data_spaceId.suffix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(data_spaceId.suffix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(appPlantFormat.prefix(2)) + String(userEnjoyUrl.suffix(5)) + "sessi" + String(main_shineTitle.suffix(5)) + "iled"))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(data_micId.suffix(4)) + "rt can" + main_insideValue.replacingOccurrences(of: "limited", with: "e")))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(user_secureId.suffix(7)) + String(userBlockPath)))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(user_expressMsg) + String(main_taMsg)))
            }
        }
    }
    ...
    private class func signalRidAbove() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(constDestroyDoinglyData.suffix(5)) + "MM-d" + String(main_unitTempMsg.suffix(6)) + String(userFlushlyMsg.prefix(8)))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(mainSpeedPrimaryContent.prefix(7)) + String(k_districtValue.suffix(4))) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(dataBasicStandUrl.prefix(4))))
        //: return outputPath
        return outputPath
    }
}
