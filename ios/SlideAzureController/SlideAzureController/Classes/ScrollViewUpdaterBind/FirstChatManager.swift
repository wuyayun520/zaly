...
/*: "Speaker On" :*/
fileprivate let main_statData:[Character] = ["S","p","e","a","k"]
fileprivate let constSkiStr:[Character] = ["e","r"," ","O","n"]

/*: "Speaker Off" :*/
fileprivate let app_permissionUrl:[Character] = ["S","p","e","a","k","e","r"," ","O","f","f"]

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let show_shallBrightPath:[Character] = ["T","R","T","C"," ","音","\u{9891}"]
fileprivate let kMissingHearMessage:String = "\u{901a}话销\u{6bc1}. "

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let mainResentStr:[Character] = ["T","R","T","C"," ","音","\u{9891}","\u{901a}","话","错","\u{8bef}","."," ","e","r","r","C","o","d","e",":"]

/*: , errMsg: :*/
fileprivate let kRemFormat:[Character] = [","," ","e","r","r","M","s","g",":"]

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let k_replacementName:[Character] = ["T","R","T","C"," ","音","频","通","\u{8bdd}","S","D","K","收","到","警","告","."," ","w","a","r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let userSimultaneouslyMsg:[Character] = [","," ","w","a","r","n","i","n","g","M","s","g"]
fileprivate let const_adminSuspendData:String = ":"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let mainPlusStr:[Character] = ["T","R","T","C"," ","音","频","通","话","\u{8fdb}","入","房","间","."," ","r","e","s"]
fileprivate let appPassMsg:[Character] = ["u","l","t",":"]

/*: , logId： :*/
fileprivate let noti_spaceTitle:[Character] = [","," ","l","o","g","I","d","："]

/*:  加入了房间" :*/
fileprivate let app_withoutStr:String = " \u{52a0}\u{5165}了房间"

/*:  离开了房间" :*/
fileprivate let show_lowPath:String = " 离\u{5f00}\u{4e86}房间"
...
private class MotivationChatConfig: NSObject {
...
class FirstChatManager: NSObject {
    ...
    private var config = MotivationChatConfig() // 音频渲染配置类
    //: private var logId = 0
...
extension FirstChatManager {
    ...
    func message(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !contrastSet()
    ...
    func assetHandsfree(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard contrastSet() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(main_statData) + String(constSkiStr)).localized : (String(app_permissionUrl)).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        fastening(isHandsfree: isHandsfree)
    }
}
...
extension FirstChatManager {
    ...
    func offPerform() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(serverCrop(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(notification(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(smoothAcross(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(alleviation(notif:)),
    ...
    @objc private func serverCrop(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func notification(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func smoothAcross(notif: Notification) {
    ...
    @objc func alleviation(notif: Notification) {
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(main_statData) + String(constSkiStr)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            fastening(isHandsfree: self.config.isHandsfree)
...
extension FirstChatManager {
    ...
    private func moveNumberoWithinAntechamberCrush() {
    ...
    private func fastening(isHandsfree: Bool) {
    ...
    private func humanBeings() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        MateThen.trackBy(msg: (String(show_shallBrightPath) + kMissingHearMessage + "logId：") + "\(logId).")
...
extension FirstChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        MateThen.trackBy(msg: (String(mainResentStr)) + "\(errCode)" + (String(kRemFormat)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        MateThen.trackBy(msg: (String(k_replacementName)) + "\(warningCode)" + (String(userSimultaneouslyMsg) + const_adminSuspendData.capitalized) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        MateThen.trackBy(msg: (String(mainPlusStr) + String(appPassMsg)) + "\(result)" + (String(noti_spaceTitle)) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
