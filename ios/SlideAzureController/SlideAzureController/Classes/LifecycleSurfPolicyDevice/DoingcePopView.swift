...
/*: "cellID" :*/
fileprivate let showAmberValue:String = "cellIDdocument social keep back except"

/*: "init(coder:) has not been implemented" :*/
fileprivate let appDevelopmentUrl:[UInt8] = [0x1b,0x1c,0x1b,0x6,0x5a,0x11,0x1d,0x16,0x17,0x0,0x48,0x5b,0x52,0x1a,0x13,0x1,0x52,0x1c,0x1d,0x6,0x52,0x10,0x17,0x17,0x1c,0x52,0x1b,0x1f,0x2,0x1e,0x17,0x1f,0x17,0x1c,0x6,0x17,0x16]

private func graduatedTable(future num: UInt8) -> UInt8 {
    return num ^ 114
}

/*: "Video Call" :*/
fileprivate let dataEnableicialName:[Character] = ["V","i","d","e","o"," ","C","a","l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let constMeanValue:String = "Videcompound control half warning drop"
fileprivate let const_beforeExecuteData:[Character] = ["l"," ","(","%","@"," ","P","o","i","n","t","s"]
fileprivate let notiShirtId:[Character] = ["/","M","i","n",")"]

/*: "Voice Call" :*/
fileprivate let k_middleName:String = "guide recordingVoice"

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let showDirectUrl:[Character] = ["V","o","i","c","e"," ","C","a","l"]
fileprivate let noti_hisStr:String = "l (filter minimize button"
fileprivate let show_helloData:String = "%@ Poplus version air port join"
fileprivate let showHoldMsg:String = "Min)fit edge discussion"

/*: "#F8F8F8" :*/
fileprivate let constManageText:String = "cease come#F8F8F8"

/*: "Cancel" :*/
fileprivate let const_talkValue:String = "Cancelbefore package op small"

/*: "#F5F5F5" :*/
fileprivate let appDeviceStr:String = "#F5F5F5return bag render build resume"

/*: "icon_lounge" :*/
fileprivate let appCombinedName:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]
...
class DoingcePopView: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: WallPopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.dialog()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.bigGallery(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.saveInColor()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.bigGallery(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.attractiveIn()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.pingfang(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(showAmberValue.prefix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + appInviteValue) : appInviteValue
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appDevelopmentUrl.map{graduatedTable(future: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func reasonPostList(cellTitleList: [String]!) {
        // 初始化
    ...
    func amplitudeModulation(type: WallPopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if DismissReactiveCompatible.share.appStatus == AfterArtSumLiteral.normal.rawValue {
                ...
                if DismissReactiveCompatible.share.loginUserMode.sex == FenceSequence.male.rawValue {
                    ...
                    let videoAttrStr = String.glyceroliseActionExpensiveness(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.outBalance(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.attractiveIn(), .font: UIFont.bigGallery(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(dataEnableicialName)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(constMeanValue.prefix(4)) + "o Cal" + String(const_beforeExecuteData) + String(notiShirtId)).unitedStates(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(k_middleName.suffix(5)) + " Call").localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(showDirectUrl) + String(noti_hisStr.prefix(3)) + String(show_helloData.prefix(5)) + "ints/" + String(showHoldMsg.prefix(4))).unitedStates(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(dataEnableicialName)).localized, (String(k_middleName.suffix(5)) + " Call").localized]
            }
        }
    ...
    public func visitorWhen() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: main_nameData - tableViewHight, width: main_logStr, height: tableViewHight), style: .plain)
        ...
        tableView.register(HeaderViewCell.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.rogueStateForRadii(width: main_logStr,
...
extension DoingcePopView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? HeaderViewCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = HeaderViewCell(style: .default, reuseIdentifier: cellID)
        }

        //: if DismissReactiveCompatible.share.loginUserMode.sex == Gender.male.rawValue,
        if DismissReactiveCompatible.share.loginUserMode.sex == FenceSequence.male.rawValue,
           //: DismissReactiveCompatible.share.loginUserMode.loungePlus == true,
           DismissReactiveCompatible.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && DismissReactiveCompatible.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && DismissReactiveCompatible.share.appStatus == AfterArtSumLiteral.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: main_logStr, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: main_logStr, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(constManageText.suffix(7))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: main_logStr, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(const_talkValue.prefix(6))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(maleProduct), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: main_logStr, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: main_logStr - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: main_logStr, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(appDeviceStr.prefix(7))))
...
class HeaderViewCell: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.locationName(name: (String(appCombinedName)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(appDeviceStr.prefix(7))))
        //: return lab
        return lab
        //: }()
    }()
