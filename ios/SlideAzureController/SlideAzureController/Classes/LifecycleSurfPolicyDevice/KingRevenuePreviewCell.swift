...
fileprivate let const_peerMsg:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func preciselyHung(awake num: UInt8) -> UInt8 {
    return num ^ 217
}
...
class KingRevenuePreviewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_peerMsg.map{preciselyHung(awake: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func just() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(heatEnergyRoot))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(deadlineWrite))
...
extension KingRevenuePreviewCell: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func heatEnergyRoot(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func deadlineWrite(tap: UIGestureRecognizer) {
    ...
    func beginIn(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.bolt(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func beyondImage(image: UIImage) {
